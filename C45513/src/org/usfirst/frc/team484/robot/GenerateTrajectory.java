...
/**
 * This class creates a new spline-based trajectory for each side of the robot 
 * to follow based on the supplied way points. Once created, the trajectories 
 * are saved using the given name.
 */
public class GenerateTrajectory {
	...
	/**
	 * Generates a trajectory with the supplied waypoints and saves it with a given name.
	...
	public static Thread execute(String name, Waypoint... points) {
		...
		//This program operates in meters. It will convert all input variables from inches to meters.
		...
		if (new File(SAVE_DIR + name + "/waypoints.csv").exists()) {
			try {
				BufferedReader fileReader = new BufferedReader(new FileReader(new File(SAVE_DIR + name + "/waypoints.csv")));
				String ln;
				int i = 0;
		...
		Thread calculationThread = new Thread() {
			public void run() {
				...
				Trajectory.Config config = new Trajectory.Config(
						FitMethod.HERMITE_QUINTIC, Config.SAMPLES_HIGH, 
				...
				try {
					...
					for (int i = 0; i < modifier.getLeftTrajectory().length() && 
						...
						pathWriter.write(segl.x + "," + segl.y + ","
								+ segr.x + "," + segr.y);
						pathWriter.newLine();
					}
					pathWriter.close();
